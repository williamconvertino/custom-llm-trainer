...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = self.W_q(x).view(B, S, self.config.n_heads, self.config.hidden_dim // self.config.n_heads)
        k = self.W_k(x).view(B, S, self.config.n_heads, self.config.hidden_dim // self.config.n_heads)
